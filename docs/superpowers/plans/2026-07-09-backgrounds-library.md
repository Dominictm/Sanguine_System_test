...
- [x] `npm run test:unit` — 324/324 зелёное
- [x] Headless Chrome: все 5 категорий рендерят верное число карточек
      (15/15/4/10/7 = 51), клик открывает модалку с верным содержимым на
      5 разных дополнениях (Ресурсы/Домен/Домитор/Часовня/Химера), 0 ошибок
      в консоли. Отдельно проверено: у части дополнений (Домен/Часовня/
      Химера) в модалке нет уровневой шкалы — подтверждено сверкой с сырым
      вики-текстом, что это отсутствие данных на самой странице-источнике
      (нет `•`-таблицы), а не баг парсера.
- [x] `impeccable` детектор по изменённым файлам — 0 находок, относящихся к правке
- [x] Коммиты по задачам, финальный прогон тестов, отчёт пользователю
