...
- **Do** адаптировать под ширину экрана через именованные брейкпоинты (`styles.css`, шапка секции `── Responsive ──`: phone ≤700px / tablet 701–1023px / desktop ≥1024px), не через fluid-типографику. `clamp()` — только для изолированных display/hero-чисел вне сетки (пример: `styles.css:3536`); всё, что живёт внутри grid/card-компонента, адаптируется брейкпоинтом. Причина — продуктовый регистр интерфейса: «responsive behavior is structural … not fluid typography».
