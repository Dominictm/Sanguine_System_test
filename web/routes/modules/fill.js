...
module.exports = function fillRouter({ makeGenerationClient, genTextWithRetry }) {
  ...
  # СУЩЕСТВУЮЩИЕ ЛОКАЦИИ — формат «Название — Район» (переиспользуй НАЗВАНИЯ БЕЗ ИЗМЕНЕНИЙ, если подходят)
  ${locCatalog}
  # ПРАВИЛО ТИПОВЫХ ЛОКАЦИЙ: не выдумывай новых названий для типовых мест (станция метро, кафе, переулок, катакомбы), если в каталоге уже есть место ТОГО ЖЕ ТИПА — используй существующее (по названию). Новое типовое место вводи ТОЛЬКО если действие переносится в район, где такого места ещё нет; тогда привяжи его к конкретному району.
  ...
  ВАЖНО — мотивация НПС: если в карточке НПС (раздел «УЧАСТНИКИ МОДУЛЯ») заполнены поля «Хочет», «Боится», «Рычаг» — НПС проактивен: в сценах он делает шаги к своей цели, его страх и рычаг — материал для конфликтов и торга, а не декорация.

  Структура сценария — ЭТАЛОННЫЙ формат, РОВНО три типа блоков верхнего уровня, без обёртки «## Сцены» вокруг сцен и без отдельного блока GM-справки (секреты — прозой внутри пролога):

  1. «## Пролог — <Название>» — как ПК втягивается в события; сюда же прозой вплетаются секреты Мастера (что произошло до сессии, тайные мотивы ключевых НПС). Подразделы: «### Описание для игрока», «### GM-подсказки» (включая проверку таймлайна, если есть конфликт — см. выше).
  2. «## Сцена 1 — <Название (место, округ)>» … «## Сцена N — …» (3–5 сцен, СРАЗУ на верхнем уровне, НЕ вкладывай их под общий заголовок). Для каждой сцены — переменный набор подразделов по смыслу (не все нужны в каждой сцене): «### Описание для игрока», при необходимости «### Бросок» / «### Бросок — <название>» (только характеристика + способность + сложность — без заранее расписанных исходов по успеху/провалу, это отыгрывает Мастер живьём за столом), «### GM-подсказки». ОБЯЗАТЕЛЬНО в КАЖДОЙ сцене (не переменный, а строго обязательный подраздел): «### Колорит» — 2-3 детали места/района/времени, которые нельзя перепутать с другим городом или эпохой.
  3. «## Финал — <Название>» — развязка, теми же переменными подразделами (например «### Раскрытие», «### GM-подсказки — финал»); «### Колорит» в финале не обязателен.
  ...
  Язык: русский. Стиль: готический нуар, VtM атмосфера.`;
      ...
      try {
        scenarioText = (await genTextWithRetry(gen, {
          system: systemPrompt, user: userPrompt, maxTokens: 8000, model: _claudeOnlyModel(gen, 'claude-opus-5'),
        })).text.trim();
      } catch (scenErr) {
  ...
  > **Название:** ${name} | **Район:** [район] | **Дополнение к адресу:** [доп. к адресу] | **Адрес:** [адрес] | **Зона:** [🟢/🟡/🔴] | **Контроль:** [фракция]
  ---
  ## 🎭 Атмосфера
  ...
  Дополнительно добавь в блок полей карточки три строки мотивации (по одной короткой фразе, из логики сценария):
  - **Хочет:** <цель НПС>
  - **Боится:** <страх НПС>
  - **Рычаг:** <чем на него можно надавить>
