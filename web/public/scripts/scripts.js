...
function emptyCharsState(belonging, lineage, status, search) {
  ...
  if (belonging === 'familiar' && !narrowed) {
    return emptyStateHtml({
      title: 'Реестр фамильяров пуст',
      body: 'Фамильяр заводится обычной карточкой персонажа. Линейку выбирают по природе существа: <b>Смертный</b> — животное-гуль или дух в зверином теле, <b>Фея / Ченджлинг</b> — химерический спутник. Дальше — «Принадлежность: Фамильяр».',
      hint: 'Хозяина впишите первым пунктом секции «Отношения» со словом «домитор» или «хозяин» — связь проступит в графе. Кнопка «+ Создать» подставит принадлежность сама.',
    });
  }
  if (narrowed) {
    return emptyStateHtml({
      title: 'Под этот отбор записей нет',
      body: 'Архив хранит персонажей, но ни один не отвечает всем условиям сразу.',
      hint: 'Ослабьте фильтр линейки, статуса или поиск по имени.',
    });
  }
  return emptyStateHtml({
    body: 'Ни одна карточка не отмечена этой принадлежностью.',
    hint: 'Принадлежность правится в карточке персонажа: вкладка «Инфо» → режим редактирования.',
  });
}
