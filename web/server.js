...
function _mockGenText(system = '', user = '') {
  const s = `${system}\n${user}`;
  // Проверяется ПЕРВЫМ: systemPrompt генерации сценария подмешивает целиком
  // module_rules.md, который сам по себе упоминает «канон»/«неканоничных» —
  // при более позднем порядке это ложно ловилось веткой
  // «непротиворечивост|канон» ниже (предназначенной для отдельной проверки
  // канона), и сценарий сохранялся как её текст-заглушка вместо сцен.
  if (/создай полный сценарий/i.test(s))
    return `## 🔒 GM-справка — закрытая информация

> Читать перед игрой. Не раскрывать игроку напрямую.

### Что произошло до начала сессии
Заглушка для автотестов: конфликт уже назревал до начала модуля.

## Пролог — Завязка

### Описание для игрока
Заглушка для автотестов: завязка сюжета.

## Сцена 1 — Место действия

### Описание для игрока
Заглушка для автотестов: первая сцена.

### Колорит
1. Заглушка деталь места.

## Финал — Развязка

### Описание для игрока
Заглушка для автотестов: развязка сюжета.

## Открытые вопросы после модуля

| Вопрос | Нить |
|---|---|
| Заглушка вопроса | Заглушка нити |
