...
const {
  RU_MONTHS_NOM, slugify, parseDiary, readPrompt, writePrompt,
...
const {
  ...
  LINEAGE_MAP, getAllCharacters, getAllLocations, findLocMdPath, listModules,
  countMdFiles, mapLimit, tableCell, readOpenThreadsRaw,
} = require('./lib/db');
...
app.post('/api/chronicles/:slug/recap', aiRateLimit, express.json(), async (req, res) => {
  try {
    const city   = reqCity(req);
    const slug   = req.params.slug;
...
app.post('/api/chronicles/:chr/modules/:mod/close', aiRateLimit, express.json(), async (req, res) => {
  try {
    const city = reqCity(req);
    const { chr, mod } = req.params;
    const modDir = path.join(chroniclesDir(city), chr, 'modules', mod);
...
app.post('/api/characters/:slug/diary/generate', aiRateLimit, express.json(), async (req, res) => {
  try {
    const city = reqCity(req);
    const slug = decodeURIComponent(req.params.slug);
...
app.post('/api/characters/:id/dialogue', aiRateLimit, express.json(), async (req, res) => {
  try {
    const city = reqCity(req);
...
app.post('/api/characters/:slug/sheet/generate', aiRateLimit, express.json(), async (req, res) => {
  try {
    const city  = reqCity(req);
    const slug  = decodeURIComponent(req.params.slug);
    const chars = await getAllCharacters(city);
...
app.post('/api/chronicles/:chr/modules/:mod/npc/:slug/sheet/generate', aiRateLimit, express.json(), async (req, res) => {
  try {
    const { chr, mod, slug } = req.params;
...
app.post('/api/canon-check', aiRateLimit, express.json(), async (req, res) => {
  try {
    const city = reqCity(req);
...
app.post('/api/characters/:slug/generate-appearance', aiRateLimit, express.json(), async (req, res) => {
  try {
...
app.post('/api/characters/:slug/generate-prompt', aiRateLimit, express.json(), async (req, res) => {
  try {
    // Validate cheap inputs BEFORE constructing a generation client (no API call needed to 404/400).
    const slug = decodeURIComponent(req.params.slug);
    const city = reqCity(req);
...
app.post('/api/characters/:slug/generate-personality', aiRateLimit, express.json(), async (req, res) => {
  try {
    const slug = decodeURIComponent(req.params.slug);
    const city = reqCity(req);
    const chars = await getAllCharacters(city);
...
app.post('/api/characters/:slug/generate-biography', aiRateLimit, express.json(), async (req, res) => {
  try {
    const slug = decodeURIComponent(req.params.slug);
    const city = reqCity(req);
    const chars = await getAllCharacters(city);
...
app.post('/api/director/propose', aiRateLimit, express.json(), async (req, res) => {
  try {
    const city = reqCity(req);
...
app.post('/api/openrouter/generate-prose', aiRateLimit, express.json(), async (req, res) => {
  try {
...
app.post('/api/claude/generate-prose', aiRateLimit, async (req, res) => {
  try {
    const stubs = Array.isArray(req.body.stubs) ? req.body.stubs : [];
    if (!stubs.length) return res.status(400).json({ ok: false, error: 'Не переданы stub-файлы.' });
