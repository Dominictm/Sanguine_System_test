...
function parseCharacter(rawContent, folderName, lineage) {
  ...
  // [ \t]* (не \s*) между «:**» и значением — \s* матчит и переводы строк, поэтому на
  // строке с ПУСТЫМ значением («- **Поле:** » — только висячий пробел, конец строки)
  // \s* проглатывал этот пробел и пустые строки после него и утаскивал (.+) аж на
  // ближайшую непустую строку дальше по карточке (например на «---»-разделитель перед
  // «## 🖼️ Изображения» — так и обнаружено: очистка «Иерархии» через
  // PUT /api/cities/:slug читалась обратно как "---"). [ \t]* держит поиск значения в
  // границах ТЕКУЩЕЙ строки, как и задумано форматом «- **Поле:** Значение» (однострочные
